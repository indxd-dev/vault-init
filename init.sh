...
read -p 'Enter VAULT_OIDC_CALLBACK_ADDR: ' input_VAULT_OIDC_CALLBACK_ADDR
VAULT_OIDC_CALLBACK_ADDR=${input_VAULT_OIDC_CALLBACK_ADDR:-$VAULT_OIDC_CALLBACK_ADDR}
if [ -z "$VAULT_OIDC_CALLBACK_ADDR" ]; then
    echo "VAULT_OIDC_CALLBACK_ADDR is not set. Exiting."
    exit 1
fi
